...
push(
    ...
    '',
    'In der ioBroker-Administration öffnet man `Objekte → viessmann.0 → set` und wählt den gewünschten Datenpunkt aus. Maßgeblich sind folgende Objektfelder:',
    '',
    '| Objektfeld | Bedeutung |',
    '| --- | --- |',
    '| `common.states` | Erlaubte Moduswerte mit menschenlesbarer Bezeichnung; ioBroker zeigt daraus normalerweise eine Auswahlliste. |',
    '| `common.min` / `common.max` | Untere und obere Grenze eines Temperaturwerts. Der Adapter verlangt zusätzlich immer eine ganze Zahl. |',
    '| `common.type` | Erwarteter ioBroker-Datentyp. Die fünf neuen A-Datenpunkte sind als `number` angelegt. |',
    '| `common.desc` | Menschenlesbare Erklärung, Wirkung, Kombination mit dem zugehörigen Modus und Rücksetzweg. |',
    '| `native.command` | Exaktes vcontrold-Schreibkommando. |',
    '| `native.address` | Exakte Viessmann-Technik-ID. Der Adapter blockiert das Schreiben, falls Kommando und Adresse nicht zur fest kodierten Policy passen. |',
    '',
    'Die XML-Metadaten allein sind nicht die einzige Schutzschicht. Dieselben Werte sind fest im Adaptercode hinterlegt. Ein nachträglich hinzugefügtes oder manipuliertes XML-SET-Kommando erhält daher nicht automatisch Schreibzugriff.',
    '',
    '### Exakte Bedeutung der fünf neuen Runtime-Kandidaten',
    '',
    '| Datenpunkt | Adresse | Wert | Erwartete Wirkung laut Viessmann-Semantik | Beenden/Rückkehr | Praktischer Status |',
    '| --- | --- | ---: | --- | --- | --- |',
    '| `set.BetriebsartExternHK1` | `A400` | `1` | Heizen mit externem Raum-Sollwert aus `A401` im normalen Heizmodus. | `A400=255` | Stärkerer Eingriff; Aktivwirkung noch kontrolliert zu testen. Eine verzögerte Zusatzheizung ist abhängig von der Anlagenlogik möglich. |',
    '| `set.BetriebsartExternHK1` | `A400` | `13` | Economy/reduzierter Heizbetrieb mit externem Raum-Sollwert aus `A401`. | `A400=255` | Bevorzugter erster Heiztest, aber Aktivwirkung und Timeout noch nicht validiert. |',
    '| `set.BetriebsartExternHK1` | `A400` | `100` | Direkte externe Vorlaufvorgabe aus `A403`; größere Teile der Heizkurvenlogik werden übersteuert. | `A400=255` | Experimentell; erst nach erfolgreichem Raum-Sollwert-Test verwenden. |',
    '| `set.BetriebsartExternHK1` | `A400` | `255` | Externe HK1-Vorgabe beenden; interne Heizkreisregelung übernimmt. | bereits Rücksetz-/Neutralwert | Neutraler Write und unmittelbarer Readback wurden real bestätigt. |',
    '| `set.RaumsollExternHK1` | `A401` | ganze Zahl `18..24 °C` | Externer HK1-Raum-Sollwert; wirkt nur mit `A400=1` oder `A400=13`. | durch `A400=255` unwirksam | Direkter Optolink-Read lieferte Fehlercode 4; Wirkung muss über effektive Temperaturen und Anlagenverhalten geprüft werden. |',
    '| `set.VorlaufsollExternHK1` | `A403` | ganze Zahl `20..45 °C` | Externer HK1-Vorlauf-Sollwert; wirkt nur mit `A400=100`. | durch `A400=255` unwirksam | Experimenteller direkter Override; direkter Optolink-Read lieferte Fehlercode 4. |',
    '| `set.WWBetriebsartExtern` | `A3C2` | `1` | Warmwasserbereitung mit externem Sollwert aus `A3C0`; interne Zeit-/Sollwertvorgaben werden während des Overrides übersteuert. | `A3C2=255` | Aktivwirkung, Timeout und möglicher Heizstabeinsatz müssen kontrolliert getestet werden. |',
    '| `set.WWBetriebsartExtern` | `A3C2` | `255` | Externe Warmwasservorgabe beenden; interne WW-Regelung übernimmt. | bereits Rücksetz-/Neutralwert | Neutraler Write und unmittelbarer Readback wurden real bestätigt. |',
    '| `set.WWSollExtern` | `A3C0` | ganze Zahl `40..50 °C` | Externer Warmwasser-Sollwert; wirkt nur mit `A3C2=1`. | durch `A3C2=255` unwirksam | Direkter Optolink-Read lieferte Fehlercode 4; Heizstabstatus bei jedem Test überwachen. |',
    '',
    '### Akzeptierte Eingabetypen',
    '',
    'Der Adapter akzeptiert ausschließlich:',
    '',
    '- echte Zahlen, zum Beispiel `21`;',
    '- numerische Strings, zum Beispiel `"21"`;',
    '- ausschließlich ganzzahlige Werte innerhalb der oben genannten Whitelist beziehungsweise Min-/Max-Grenzen.',
    '',
    'Der Adapter blockiert insbesondere:',
    '',
    '- Boolean-Werte `true` und `false`;',
    '- Dezimalwerte wie `21.5`;',
    '- leere Strings, `null` und `undefined`;',
    '- Arrays und Objekte;',
    '- nicht freigegebene Modi wie `A400=8` oder `A3C2=6`;',
    '- Temperaturen außerhalb der konfigurierten Grenzen.',
    '',
    'Ein blockierter Wert wird nicht an vcontrold weitergegeben. Im ioBroker-State kann der versuchte Wert trotzdem zunächst mit `ack=false` sichtbar bleiben; maßgeblich sind deshalb zusätzlich das `ack`-Flag und der Adapterlogeintrag `Blocked write ...`.',
    '',
    '### Bedeutung des `ack`-Flags',
    '',
    '| Zustand | Bedeutung |',
    '| --- | --- |',
    '| `ack=false` | Schreibwunsch des Benutzers oder einer Automation. Noch keine Bestätigung durch vcontrold; der Wert kann auch von der Adapter-Policy blockiert worden sein. |',
    '| `ack=true` | vcontrold hat das Kommando mit `OK` bestätigt und der Adapter hat den State quittiert. |',
    '',
    '`ack=true` beweist nur die Annahme des Kommandos durch vcontrold. Es beweist noch nicht die erwartete thermische Wirkung, ein korrektes Heartbeat-Verhalten oder eine reine RAM-Speicherung. Diese Punkte werden erst durch Readback, mehrere Anlagenmesswerte, Timeout und Neustart-/Power-Cycle-Tests bestätigt.',
    '',
    '### Konkrete Schreibfolgen',
    '',
    '#### 1. Bevorzugter erster Heiztest: Economy mit Raum-Sollwert',
    '',
    '```text',
    '1. viessmann.0.set.RaumsollExternHK1 = 21',
    '2. viessmann.0.set.BetriebsartExternHK1 = 13',
    '3. Wirkung über Temperaturen, Verdichter und Heizstabzustände beobachten',
    '4. viessmann.0.set.BetriebsartExternHK1 = 255',
    '```',
    '',
    '#### 2. Experimenteller Vorlauf-Override',
    '',
    '```text',
    '1. viessmann.0.set.VorlaufsollExternHK1 = 30',
    '2. viessmann.0.set.BetriebsartExternHK1 = 100',
    '3. Vorlauf, Rücklauf, Verdichterleistung und Heizstabzustände beobachten',
    '4. viessmann.0.set.BetriebsartExternHK1 = 255',
    '```',
    '',
    '#### 3. Externe Warmwasserbereitung',
    '',
    '```text',
    '1. viessmann.0.set.WWSollExtern = 45',
    '2. viessmann.0.set.WWBetriebsartExtern = 1',
    '3. Warmwassertemperatur, Verdichter, 0488, 0489, 048A und 1909 beobachten',
    '4. viessmann.0.set.WWBetriebsartExtern = 255',
    '```',
    '',
    'Bei jedem Test gilt: zuerst den Sollwert setzen, danach den Modus aktivieren. Zum Beenden wird ausschließlich der Modus auf `255` zurückgesetzt. Bei unerwartetem Heizstabeinsatz, Fehlerstatus oder unplausiblen Temperaturen sofort auf `255` zurückkehren.',
    '',
